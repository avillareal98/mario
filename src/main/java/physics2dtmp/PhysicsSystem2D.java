...
public class PhysicsSystem2D {
    ...
    private void applyImpluse(Rigidbody2D a, Rigidbody2D b, CollisionManifold m) {
        ...
        Vector2f impluse = new Vector2f(relativeNormal).mul(j);
        a.setVelocity(
                new Vector2f(a.getVelocity()).add(new Vector2f(impluse).mul(invMass1).mul(-1f)));
        b.setVelocity(
                new Vector2f(b.getVelocity()).add(new Vector2f(impluse).mul(invMass2).mul(1f)));
    }
